...
export function useFormRules(formData?: Recordable) {
  const getAccountFormRule = computed(() => createRule("输入您的账号"));
  const getPasswordFormRule = computed(() => createRule("输入您的密码"));
  const getSmsFormRule = computed(() => createRule("输入短信验证码"));
  const getMobileFormRule = computed(() => createRule("输入您的手机号码"));
