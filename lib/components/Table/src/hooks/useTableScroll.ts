...
export function useTableScroll(
  ...
  const { start, stop } = useWindowSizeFn(debounceRedoHeight);

  onMounted(() => {
    unref(getCanResize) && start();
  });
  ...
  function calcModalHeight(tableEl: Element) {
    ...
    while (modalElIterator !== document.body) {
      ...
      if (modalElIterator.classList.contains("ant-modal")) {
        ...
        modalIsFullscreen =
          modalWrapEl?.classList.contains("fullscreen-modal") ?? false;
        break;
      }
