...
export default defineComponent({
  ...
  setup(props) {
    ...
    function handleOptionsChange(options: never[]) {
      ...
      if (component === "ApiTreeSelect") {
        ...
        listOptions = listOptions.map((item: { [x: string]: any }) => {
          return {
            label: item[title],
            value: item[value],
          };
    ...
    function initCbs(
      ...
      handle: {
        (needEmit?: boolean, valid?: boolean): Promise<false | undefined>;
        (): Promise<boolean>;
        (): void;
      }
    ) {
    ...
    if (props.record) {
      initCbs("submitCbs", handleSubmit as any);
      initCbs("validCbs", handleSubmitRule as any);
      initCbs("cancelCbs", handleCancel as any);
      ...
      props.record.onCancelEdit = () => {
        isArray(props.record?.cancelCbs) &&
          props.record?.cancelCbs.forEach((fn: () => any) => fn());
      };
      props.record.onSubmitEdit = async () => {
        if (isArray(props.record?.submitCbs)) {
          if (!props.record?.onValid?.()) return;
          const submitFns = props.record?.submitCbs || [];
          submitFns.forEach((fn: (arg0: boolean, arg1: boolean) => any) =>
            fn(false, false)
          );
